...
let frameworksListHtml = `<main>
    <section class="category">
        <h2>Frameworks</h2>`;

frameworks.sort((a, b) => a.name.localeCompare(b.name));
frameworks.forEach((fram, index) => {
  const letter = fram.name[0];
  // On affiche L'ancre de la lettre
  if (index == 0 || frameworks[index - 1].name[0] != letter) {
    frameworksListHtml += `<h3 class="letter" data-letter="${letter}">${letter}</h3>`;
  }

  frameworksListHtml += `
  <section class="card">
      <div class="card-header">
          <h3 class="card-title">${fram.name}</h3>
          <button class="card-toggle">Voir</button>
      </div>
      <div class="card-content">
          <p class="card-definition">${
            fram.definition || `Définition du framework ${fram.name}`
          }</p>
          ${
            fram.example
              ? `<p class="card-example">Exemple de code ${fram.name} :</p>
          <pre><code>${fram.example}</code></pre>`
              : ""
          }
      </div>
  </section>`;
...
frameworksListHtml += "</section></main>";

// On ajoute le contenu à la page
const categorySection = document.querySelector(".category");
if (categorySection) {
  categorySection.innerHTML = frameworksListHtml;
}
