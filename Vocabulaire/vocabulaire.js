...
let vocabulaireListHtml = `<main>
    <section class="category">
        <h2>Langages de Programmation</h2>`;

vocabulaire.sort((a, b) => a.name.localeCompare(b.name));
vocabulaire.forEach((vocab, index) => {
  const letter = vocab.name[0];
  // On affiche L'ancre de la lettre
  if (index == 0 || vocabulaire[index - 1].name[0] != letter) {
    vocabulaireListHtml += `<h3 class="letter" data-letter="${letter}">${letter}</h3>`;
  }

  vocabulaireListHtml += `
  <section class="card">
      <div class="card-header">
          <h3 class="card-title">${vocab.name}</h3>
          <button class="card-toggle">Voir</button>
      </div>
      <div class="card-content">
          <p class="card-definition">${
            vocab.definition || `Définition de la notion ${vocab.name}`
          }</p>
          ${
            vocab.example
              ? `<p class="card-example">Exemple de code ${vocab.name} :</p>
          <pre><code>${vocab.example}</code></pre>`
              : ""
          }
      </div>
  </section>`;
...
vocabulaireListHtml += "</section></main>";

// On ajoute le contenu à la page
const categorySection = document.querySelector(".category");
if (categorySection) {
  categorySection.innerHTML = vocabulaireListHtml;
}
