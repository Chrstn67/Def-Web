...
let languageListHtml = `<main>
    <section class="category">
        <h2>Langages de Programmation</h2>`

languages.sort((a, b) => a.name.localeCompare(b.name))
languages.forEach((lang, index) => {
  const letter = lang.name[0]
  // On affiche L'ancre de la lettre
  if (index == 0 || languages[index - 1].name[0] != letter) {
    languageListHtml += `<h3 class="letter" data-letter="${letter}">${letter}</h3>`
  }
  ...
  <section class="card">
      ...
      <div class="card-content">
          <p class="card-definition">${
            lang.definition || `Définition du langage ${lang.name}`
          }</p>
          ${
            lang.example
              ? `<p class="card-example">Exemple de code ${lang.name} :</p>
...
languageListHtml += "</section></main>"

// On ajoute le contenu à la page
const categorySection = document.querySelector(".category")
if (categorySection) {
  categorySection.innerHTML = languageListHtml
}
...
searchForm.addEventListener("submit", (event) => {
  ...
  const searchTerm = document.getElementById("searchNotion").value.toLowerCase()
